...
test("multiple HTML uploads create one named task that survives restart", async () => {
  ...
  try {
    ...
    assert.equal(payload.project.editorRuntimeVersion, 36);
  ...
  legacyMeta.editorRuntimeVersion = 20;
  ...
  try {
    ...
    assert.equal(payload.projects[0].editorRuntimeVersion, 36);
    ...
    assert.match(upgradedEditable, /layout-marquee/);
