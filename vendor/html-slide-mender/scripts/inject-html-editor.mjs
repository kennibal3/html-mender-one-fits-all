...
const interactionRuntimePath = resolve(skillDir, "assets/html-slide-mender-interactions.js");
...
async function main() {
  ...
  const interactionRuntime = await readFile(interactionRuntimePath, "utf8");
  ...
  const editableHtml = injectBeforeBodyEnd(
    cleanedHtml,
    buildBodyInjection(runtime, interactionRuntime, options, cleanedHtml)
  );
